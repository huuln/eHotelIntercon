	  ...
	  private String loadDataSubjectXML(Vector vRs) throws IOException{
		    ...
		    if (vRs.size() != 0)
		    {
		      ...
		      {		    	
		        ...
		        urlpicicon = (String)vRs.get(i + 4);
		        status = new Integer(((String)vRs.get(i + 5)).trim()).intValue();                
		        subject = new ModSubject();
		        level = (String)vRs.get(i + 6);
		        obj.put("id",idSubject);
		        obj.put("subjectname",UnicodeConverter.encodeUnicode(nameSubject));
		        obj.put("desc",def);
		        obj.put("iconfocusloss",urlpic);
			    obj.put("iconfocuson",urlpicicon);
